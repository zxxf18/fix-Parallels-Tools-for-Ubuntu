                          ...
                          void *tgt_path, int tgt_len)
{
	...
	init_tg_buffer(&Req.Buffer[0], src_path, src_len, 0, 0);
	init_tg_buffer(&Req.Buffer[1], tgt_path, tgt_len, 1, 0);
	init_req_desc(&sdesc, &Req.Req, NULL, &Req.Buffer[0]);
	pdev = PRLFS_SB(sb)->pdev;
	ret = call_tg_sync(pdev, &sdesc);
	if ((ret == 0) && (Req.Req.Status != TG_STATUS_SUCCESS))
                         ...
                         const void *tgt_path, int tgt_len)
{
	...
	init_tg_buffer(&Req.Buffer[0], &PRLFS_SB(sb)->sfid, sizeof(PRLFS_SB(sb)->sfid), 0, 0);
	init_tg_buffer(&Req.Buffer[1], (void *)src_path, src_len, 0, 0);
	init_tg_buffer(&Req.Buffer[2], (void *)tgt_path, tgt_len, 0, 0);
	init_req_desc(&sdesc, &Req.Req, NULL, &Req.Buffer[0]);
	pdev = PRLFS_SB(sb)->pdev;
	ret = call_tg_sync(pdev, &sdesc);
	if ((ret == 0) && (Req.Req.Status != TG_STATUS_SUCCESS))
